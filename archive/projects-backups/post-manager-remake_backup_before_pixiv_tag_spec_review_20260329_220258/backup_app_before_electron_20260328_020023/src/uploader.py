...
from config import Paths, BrowserConfig, CSV_PATH, USER_DATA_DIR
from utils import launch_browser

def validate_paths():
    """必要なファイルやディレクトリの存在確認"""
    if not Paths.CSV.exists():
        print(f"Error: CSV file not found at {Paths.CSV}")
        return False
    if not Paths.BROWSER_PROFILE.exists():
        print(f"Creating user data directory at {Paths.BROWSER_PROFILE}")
        ...
        if schedule:
            ...
            try:
                ...
                if reserve_check.count() > 0:
                    ...
                    # Pixiv公式準拠：30分刻みに丸める（00:00, 00:30, 01:00, ...）
                    try:
                        time_parts = time_str.split(':')
                        hour = int(time_parts[0])
                        minute = int(time_parts[1]) if len(time_parts) > 1 else 0
                        # 30分刻みに丸める（15分未満→00, 15分以上45分未満→30, 45分以上→次の時間の00）
                        if minute < 15:
                            rounded_minute = 0
                        elif minute < 45:
                            rounded_minute = 30
                            ...
                            rounded_minute = 0
                            hour = (hour + 1) % 24
                        time_str = f"{hour:02d}:{rounded_minute:02d}"
                        print(f"  -> Time rounded to 30-min interval: {time_str}")
                    except ValueError:
