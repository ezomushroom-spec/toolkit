...
const UI = {
    ...
    setupPixivTagSupport() {
        const watchedInputs = [
            ...
            document.getElementById('edit-caption-pixiv'),
            this.elements.pixivTagInput
        ];

        watchedInputs.forEach(input => {
            if (!input) return;
            input.addEventListener('input', () => this.queuePixivTagSuggestions());
            input.addEventListener('change', () => this.queuePixivTagSuggestions(true));
        });
    },
    ...
    async refreshPixivTagSuggestions() {
        ...
        const captionInput = document.getElementById('edit-caption-pixiv');
        const tagInput = this.elements.pixivTagInput;

        if (!titleInput || !folderInput || !captionInput || !tagInput) {
            return;
        }

        const requestToken = ++state.pixivTagRequestToken;
        this.setPixivTagStatus('Pixivタグ候補を読み込み中...', 'info');

        try {
            const result = await API.getPixivTagSuggestions({
                title: titleInput.value || '',
                caption_pixiv: captionInput.value || '',
                target_folder: folderInput.value || '',
            ...
            this.setPixivTagStatus('Pixivタグ候補を読み込めませんでした。手入力はそのまま利用できます。', 'warning');
        }
    },
    ...
    renderPixivTagSuggestions(suggestions) {
        ...
        if (!suggestions.length) {
            this.setPixivTagStatus('候補がまだありません。Pixivタグは手入力で続けられます。', 'info');
            return;
        }
        ...
        suggestions.forEach(item => {
            ...
            chip.title = this.getPixivTagSourceLabel(item.source);
            chip.disabled = isSelected;
            chip.addEventListener('click', () => this.appendPixivTag(item.tag));
            container.appendChild(chip);
        });

        this.setPixivTagStatus('候補をクリックすると Pixivタグ に追加されます。', 'info');
    },
...
const FolderBrowser = {
    ...
    updateDropZoneHint() {
        ...
        if (desktopBridge && typeof desktopBridge.pickFolder === 'function') {
            ...
            hint.textContent = 'Electron では D&D を主入力として使えます。参照ボタンからネイティブ選択も可能です。';
            return;
        }

        title.textContent = '📂 フォルダパス入力の補助';
