...
class ParamPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.slider_str.valueChanged.connect(self.spin_str.setValue)
        self.slider_str.valueChanged.connect(lambda _: self.paramChanged.emit())
        self.spin_str.valueChanged.connect(self.slider_str.setValue)
        self.spin_str.valueChanged.connect(lambda _: self.paramChanged.emit())
        ...
        self.slider_margin.valueChanged.connect(self.spin_margin.setValue)
        self.slider_margin.valueChanged.connect(lambda _: self.paramChanged.emit())
        self.spin_margin.valueChanged.connect(self.slider_margin.setValue)
        self.spin_margin.valueChanged.connect(lambda _: self.paramChanged.emit())
    ...
    def build_class_settings(self, target_classes: list):
        ...
        for cls_info in target_classes:
            ...
            slider.valueChanged.connect(lambda value, s=spin: s.setValue(value / 100.0))
            spin.valueChanged.connect(
                lambda value, sl=slider: sl.setValue(int(value * 100))
            )
            slider.valueChanged.connect(lambda _: self.paramChanged.emit())
            spin.valueChanged.connect(lambda _: self.paramChanged.emit())
