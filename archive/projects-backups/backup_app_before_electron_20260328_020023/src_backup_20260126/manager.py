...
from uploader_discord import send_discord_notification, load_secrets
...
def apply_template(template_str, task):
    """テンプレート文字列にtaskの値を埋め込む"""
    if not template_str:
        return ""
    try:
        # task辞書の値をそのままフォーマット引数として渡す
...
def run_clean(tasks):
    ...
    for task in tasks:
        ...
        if not os.path.isabs(target_folder):
            potential_path = os.path.join(BASE_DIR, target_folder)
        ...
        # 既存Zipの確認
        # Zip名の生成ロジック: 常にフォルダ名準拠に統一 (preprocessor.pyと合わせる)
        ...
        if os.path.exists(zip_path):
            ...
            # デフォルトでスキップ、または確認
            # ユーザー要望: 「cleaning images もファイルチェックしてスキップ判定に入れてくれ」
            ...
            while True:
                ...
                if choice in ['s', 'skip']:
                    ...
                    # zip_pathを返さないと呼び出し元で後続処理(MEGA判定など)に影響する場合があるが、
                    # run_clean内では task['count'] セットが主目的。
                    # ただし loop continue するので、clean_and_zip は呼ばれない。
                    # これでOK。
                    break
                elif choice in ['o', 'overwrite']:
                    # 上書きのために既存Zip削除などが必要ならここでやるが、clean_and_zipが上書きモードならOK
                    # clean_and_zipは tempフォルダ削除 -> Zip上書き保存 なのでそのままで良い
                    # clean_and_zipを呼ぶためにループを抜けるのではなく、フラグ管理するか、elseへ。
                    # ここでは break せず、ifの外に出る必要がある。
                    # しかし continue しているので、break すると外に出て clean_and_zip が呼ばれる。
                    break
                else:
...
def main():
    ...
    parser.add_argument("--clean", action="store_true", help="Step 1: Clean metadata and create Zip")
    parser.add_argument("--mega", action="store_true", help="Step 2: Upload Zip to MEGA")
    parser.add_argument("--pixiv", action="store_true", help="Step 3: Upload to Pixiv")
    parser.add_argument("--patreon", action="store_true", help="Step 4: Upload to Patreon")
    parser.add_argument("--discord", action="store_true", help="Step 5: Notify Discord")
