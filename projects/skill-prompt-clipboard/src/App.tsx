...
const wildcardGroups: WildcardGroup[] = [
  {
    id: 'lighting',
    token: '__lighting__',
    label: '光',
    options: ['soft morning light', 'golden hour backlight', 'diffused window light', 'rainy neon reflection'],
  },
  {
    id: 'camera',
    token: '__camera__',
    label: '構図',
    options: ['close-up portrait', 'wide establishing shot', 'rear three-quarter view', 'low angle composition'],
  },
  {
    id: 'mood',
    token: '__mood__',
    label: '空気感',
    options: ['quiet and calm mood', 'nostalgic atmosphere', 'dreamy and soft mood', 'dramatic but restrained mood'],
  },
]

const initialSituation = imagePromptSituations[0]
const initialVariant = initialSituation.variants[0]
...
function App() {
  const [activeAttribute, setActiveAttribute] = useState<ImagePromptAttribute>(initialSituation.attribute)
  const [selectedSituationId, setSelectedSituationId] = useState(initialSituation.id)
  const [selectedVariantId, setSelectedVariantId] = useState(initialVariant.id)
  const [draftPrompt, setDraftPrompt] = useState(initialVariant.prompt)
  const [status, setStatus] = useState('コピー待機中')
  const [errorMessage, setErrorMessage] = useState('')
  const [isCopying, setIsCopying] = useState(false)

  const visibleSituations = useMemo(
    () => imagePromptSituations.filter((situation) => situation.attribute === activeAttribute),
    [activeAttribute],
  )

  const selectedSituation =
    imagePromptSituations.find((situation) => situation.id === selectedSituationId) ?? initialSituation
    ...
    <main className="app-shell">
      ...
      <section className="studio-summary" aria-label="現在の作業状態">
        ...
        <div>
          ...
          <strong>{wildcardGroups.length} groups</strong>
        </div>
      </section>
