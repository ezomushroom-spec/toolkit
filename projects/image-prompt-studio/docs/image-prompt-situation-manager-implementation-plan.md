...
- 目的: `image-prompt-studio` を画像生成プロンプト管理向けの新規 project として扱い、画像生成プロンプトを situation 単位で管理できるようにする。
- 対象 project: `projects/image-prompt-studio`
...
- app 名称変更は初回では行わない。
- `E:\自作アプリ集\新しいフォルダー (2)` の正本情報は `docs/reference-sd-prompt-manager-findings.md` に要点を整理済み。
- 正本から直接取り込む場合は、まず txt wildcard の読み取り専用インポートから検討する。
