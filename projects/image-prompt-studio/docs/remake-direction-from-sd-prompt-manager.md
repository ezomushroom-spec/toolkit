# SD Prompt Manager 正本再構築方針

## 1. 結論

- 既存 React app へ場当たり的に機能追加するより、`E:\自作アプリ集\新しいフォルダー (2)` を現行正本として、画像生成プロンプト管理ツールを再構築する方が建設的。
- 正本の Python GUI をそのまま移植するのではなく、実データ構造と運用機能を引き継ぎ、UI は今後の拡張に耐える形へ整理する。
...
- 既存資産の扱い: 正本の SQLite schema、wildcard ファイル、タグ検索、prompt / negative prompt 分離を優先して確認する。
...
| B | 正本 Python app を修理・整理 | SQLite と wildcard 処理をそのまま使える | UI 改善と保守性に限界が出やすい | 中 |
| C | Python app を現行正本にして React app を再構築 | UI を作り直しつつ、実データ構造を取り込める | 初期設計が必要 | 高 |

### 2-3. 推奨案

- 推奨する構成: 候補 C。React app をベースに、正本のデータ構造と運用機能を段階的に取り込む。
- 推奨理由: プロンプト管理は UI の見通し、検索、編集、コピーの操作性が重要で、React の方が画面構成を整理しやすい。一方で、正本の SQLite と wildcard 管理は実運用の知見として保持する。
...
| React app の保存 | `localStorage` 保存を導入済み | 次は import / export を検討 |
| React app の negative prompt | 分離欄を導入済み | 次はツール別コピー形式を検討 |
| React app の wildcard | 正本 `data/wildcards` から確認済み txt 3 件を読み取り専用で取り込み済み | 次は対象ファイル拡張とランダムプレビューを検討 |
...
1. `ImagePromptVariant` に `negativePrompt` を追加する。完了。
2. `docs/ui-design-direction.md` の方針に沿って、画面を作業用スタジオ型に再整理する。
3. `localStorage` で situation / variant の保存を入れる。完了。
4. txt wildcard を JSON へ変換する読み取り専用 importer を作る。完了。
5. wildcard 一覧で `__name__` 構文コピー、候補数、候補プレビューを表示する。完了。
6. タグ DB 検索は別フェーズで、SQLite 直接利用か変換済み軽量 index かを比較する。

## 10. 2026-04-07 wildcard 取り込み方針

- 取り込み元: `E:\自作アプリ集\新しいフォルダー (2)\data\wildcards`
- 実装先: `scripts/import-canonical-wildcards.mjs` と `src/generated/canonicalWildcards.ts`
- 正本への副作用: なし。読み取り専用で参照し、正本 Python app や wildcard ファイルには書き込まない。
- 初回取り込み対象: `school komono.txt`、`tipo_location.txt`、`view.txt`
- 除外方針: 未確認の txt は `汎用候補の確認が未完了のため初回除外`、txt 以外は `txt以外のため初回除外` として summary に集約する。
- UI 反映: 取り込み元 path、取り込み group 数、除外 summary、各 group の token、候補数、source file、候補プレビュー、token コピーを表示する。
