...
| 意図ラベル | 内部対応 | 使うとき |
|---|---|---|
| `skill評価` | `empirical-prompt-tuning` | 新規 skill や大幅改訂直後の skill / prompt / guidance section を、評価シナリオと固定 checklist で見直したいとき |
...
### 4.14 skill や prompt を評価プロトコルで見直したい

```text
skill評価: この skill の description と body を見直し、scenario と checklist を作って。
```
...
- `empirical-prompt-tuning` は上位規約ではなく、既存 instruction の品質保証プロトコルとして扱う
- 小規模修正、局所的な文言修正、既存構成内の限定変更では skill を省略してよい
- 中規模以上、方式比較、構成変更、再構築、重い不具合調査では skill を優先して使う
